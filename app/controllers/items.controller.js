...
exports.create = (req, res) => {
   ...
   const item = new Item({
      ...
      product: {
         ...
         price: req.body.price ? req.body.published : 2.0,
...
exports.createNewProdect = (req, res) => {
   ...
   const product = new Object({
      title: req.body.title ? req.body.title : 'hhhhh',
      price: req.body.price ? req.body.price : 2.0,
      image: req.body.image
         ? req.body.image
         : 'https://static.remove.bg/remove-bg-web/a6eefcd21dff1bbc2448264c32f7b48d7380cb17/assets/start_remove-c851bdf8d3127a24e2d137a55b1b427378cd17385b01aec6e59d5d4b5f39d2ec.png',
      discraption: req.body.discraption ? req.body.discraption : 'true',
   ...
   Item.findOneAndUpdate({ _id: id }, { $push: { product: product } })
      .then((data) => {
         res.send({massege:"prodect has been added successfully"});
      })
      .catch((err) => {
         res.status(500).send({
            message:
   ...
   Item.deleteMany({})
      ...
      .catch((err) => {
         res.status(500).send({
            message:
               err.message ||
               'Some error occurred while removing all Item.',
         });
      });
};
